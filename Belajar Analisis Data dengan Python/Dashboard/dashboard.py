...
dataset_path = os.path.join(base_dir, "Dataset", "PRSA_Data_Aotizhongxin_20130301-20170228.csv")

# Judul Dashboard
st.title("Dashboard Analisis Kualitas Udara di Aotizhongxin")

# Sidebar
st.sidebar.image(logo_path, caption="Analisis Kualitas Udara", use_container_width=True)
...
uploaded_file = st.file_uploader("Unggah file CSV lain (opsional)", type="csv")

# Gunakan dataset default dari dataset_path jika tidak ada file yang diunggah
if uploaded_file is not None:
    df = pd.read_csv(uploaded_file)
    st.write("Dataset dari file yang diunggah berhasil dimuat:")
else:
    try:
        df = pd.read_csv(dataset_path)
        st.write("Dataset default berhasil dimuat dari:", dataset_path)
    except FileNotFoundError:
        st.error("File default tidak ditemukan di:", dataset_path)
        st.stop()
